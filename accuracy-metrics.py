...
def initializeCenters(df, k):
    ...
    centers = [list(df.iloc[idx][7:9]) for idx in random_indices]
...
def computeCenter(df, k, cluster_labels):
    ...
    for i in range(k):
        for idx, val in enumerate(cluster_labels):
            if val == i:
                print('|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||')
                print(df.iloc[idx][7:9])
                print('|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||')
                data_points.append(list(df.iloc[idx][7:9]))
...
def assignCluster(df, k, cluster_centers):
    ...
    for i in range(len(df)):
        distances = [euclidean_distance(list(df.iloc[i][7:9]), center) for center in cluster_centers]
...
k = 2
MAX_ITER = 100 
df_full = pd.read_csv('a854756.csv')
